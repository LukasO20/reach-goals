...
export const ManageModelProvider = ({ children }) => {
    ...
    const addToSelectedModel = useCallback(
        ({ id, name, type, color, custom }) => {
            if (!id || !name)
                return console.error(
                    'The model object must have an id and a name property'
                )
            if (typeof type !== 'string')
                return console.error(
                    'The type is necessary and should be a string value. Did you send something like "tag" or "assignment"?'
                )
            ...
            setModel((prevModel) => {
                const alreadyExists = prevModel.selectedModel[type].some(
                    (item) => (item.id ?? item.tagID) === id
                )
    ...
    const removeFromSelectedModel = useCallback(({ id, type }) => {
        setModel((prevModel) => {
            const updatedSelectedModel = prevModel.selectedModel[type].filter(
                (item) => (item.id ?? item.tagID) !== id
            )
            return {
                ...prevModel,
    ...
    const updateActiveModel = useCallback(
        ({ keyObject, value, type = 'single', action = 'add' }) => {
            if (!keyObject)
                return console.error(
                    'To update a activeModel is necessary a key'
                )

            const submitToAdd = (prevValue) => {
                return type === 'array'
                    ? [...(Array.isArray(prevValue) ? prevValue : []), value]
                    : value
            }

            const submitToRemove = (prevValue) => {
                if (type === 'array') {
                    const currentList = Array.isArray(prevValue)
                        ? prevValue
                        : []
            ...
            setModel((prevModel) => {
                ...
                const notExists = Array.isArray(prevValue)
                    ? !prevValue.some(
                          (item) =>
                              (item.id ?? item.tagID) ===
                              (value.id ?? value.tagID)
                      )
                    : true
                const newValue =
                    action === 'add' && notExists
                        ? submitToAdd(prevValue)
                        : action === 'remove'
                          ? submitToRemove(prevValue)
                          : prevValue
    ...
    const updateDataModel = useCallback(({ data, type, scope }) => {
        if (typeof type !== 'string' || type === '')
            return console.error('To update a dataModel is necessary a type')
        if (typeof scope !== 'string' || scope === '')
            return console.error(
                'To update a dataModel is necessary a typesource'
            )
        ...
        setModel((prevModel) => ({
            ...
            dataModel: {
                ...
                [type]: {
                    ...
                    [scope]: {
                        ...prevModel.dataModel[type][scope],
                        data,
                    },
                },
            },
