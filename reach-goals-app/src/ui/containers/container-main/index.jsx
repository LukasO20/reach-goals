...
const ContainerMain = () => {
    ...
    const moreDropdown = useButtonDropdown({
        ...
        actions: {
            setterUseSwitchLayout: (target) => setUserConfigLayout(target),
        },
    })
                        ...
                        {isPageCalendar && (
                            <div className='month-picker'>
                                <Tooltip title='Previous Month'>
                                    <ButtonAction
                                        classBtn='month-previous small circle'
                                ...
                                <Tooltip title='Next Month'>
                                    <ButtonAction
                                        classBtn='month-next small circle'
                        ...
                        {isPageHome && (
                            <>
                                <Tooltip title='Column Layout'>
                                    <ButtonAction
                                        classBtn={`plan-round max-width goal ${isSwitchColumn ? 'active' : ''}`}
                                        icon='icon-columns'
                                        switchLayout={switchLayoutMap({
                                            area: 'page',
                                            layout: {
                                                pageName: layout.page.pageName,
                                                layoutName: 'column',
                                            },
                                        })}
                                ...
                                <Tooltip title='Chart Layout'>
                                    <ButtonAction
                                        classBtn={`plan-round max-width chart ${isSwichChart ? 'active' : ''}`}
                                        icon='icon-pie-chart'
                                        switchLayout={switchLayoutMap({
                                            area: 'page',
                                            layout: {
                        ...
                        {!isPageCalendar && hasSelectedModel && (
                            <ButtonCheckbox
                                ...
                                checkbox={buildCheckboxMap({
                                    checkboxIDMain: `checkbox-${layout.page.layoutName}`,
                                    scope: 'page',
                                })}
                                title='Select all'
                            />
                        )}
                        {isPageObjectives && <ObjectivesStatus />}
                    </div>
                    <div>
                        <ButtonDropdown
                        ...
                        />
                        {(isPageCalendar || isPageHome || isPageObjectives) &&
                            !isSwichChart && (
                                <ButtonDropdown
