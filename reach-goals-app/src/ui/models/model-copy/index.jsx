...
import { removeFromSelectedModelMap } from '../../../utils/mapping/mapping-utils-provider.js'
...
const ModelCopy = ({ propsReference = safePropsReference, region }) => {
    ...
    const { model, removeFromSelectedModel, updateActiveModel } =
        useManageModel()
    ...
    const removeElDOMClick = ({ id, tagID }, type) => {
        ...
        if (!type) {
            return console.error(
                'No type provided for model copy delete action. Send a goal, assignment or tag type'
            )
        }
