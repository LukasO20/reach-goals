...
const RelationCard = ({ checkboxState = checkboxMap, data = [] }) => {
    ...
    const { remove, removing, removeSuccess, removingVariables } =
        useTagProvider()
        ...
        setOpenCard((prev) => {
            ...
            if (exists)
                return prev.map((card) =>
                    card.id === item.id ? { ...card, open: !card.open } : card
                )
    ...
    const editTag = (id) => {
        try {
            setModel((prev) => ({ ...prev, mainModelID: id, typeModel: 'tag' }))
        } catch (error) {
        ...
        .map((item) => {
            ...
            const assignment = {
                assignments: assignments
                    .filter((assignmentItem) => assignmentItem.assignment)
                    .map((assignmentItem) => {
                        return {
            ...
            const quantityGoalMessage =
                goal.goals.length > 1
                    ? `${goal.goals.length} Goals`
                    : `${goal.goals.length} Goal`
            const quantityAssigmentMessage =
                assignment.assignments.length > 1
                    ? `${assignment.assignments.length} Assignments`
                    : `${assignment.assignments.length} Assignment`
            ...
            const isOpen = openCard.some(
                (card) => card.id === item.id && card.open === true
            )
                ? 'open'
                : ''
            const isRemoving =
                !!removing && removingVariables === item.id ? 'pending' : ''
            const isSelected = selectedCheckboxList.includes(
                `checkbox-${item.id}`
            )
                ? 'selected'
                : ''
            ...
            return (
                <div
                    className={tagCardRelationClass}
                    key={item.id}
                ...
                >
                    <div
                        className='head'
                        onClick={() =>
                            hasSomeRelation ? handleSetOpenCard(item) : {}
                        }
                    >
                        <label>
                            <ButtonCheckbox
                        ...
                        {hasGoal && (
                            <div className='content goal'>
                                <label className='message'>
                                    ...
                                    <label>{quantityGoalMessage}</label>
                                </label>
                                <Goal
                                    source={goal}
                        ...
                        {hasAssignment && (
                            <div className='content assignment'>
                                <label className='message'>
                                    ...
                                    <label>{quantityAssigmentMessage}</label>
                                </label>
                                <Assignment
                                    source={assignment}
                                    display={displayModesProps}
