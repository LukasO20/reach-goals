...
export const formatDate = (model) => {
    ...
    const formatInput = (input) => {
        if (input && !moment(input, moment.ISO_8601, true).isValid()) {
            ...
            if (!validDate.isValid()) {
                console.error(
                    'THIS IS IS INVALID DATE (put an indicator input date error to users see)'
                )
            }
    ...
    const formatStart = start
        ? formatInput(start) || formatInputISO(start)
        : undefined
...
export const hasRequiredProps = (props, requiredKeys = []) => {
 ...
 * @returns {Array}
 */
export const sortedMap = (data = [], attribute) => {
...
export const cx = (str = '') => {
    return str
        .replace(/\b(undefined|null|false)\b/g, '')
        .replace(/\s+/g, ' ')
        .trim()
}
 ...
 * @returns {string}
 */
export const calculatePercent = (value, quantity) => {
 ...
 * @param {string} pY
 */
export const getTransform = (pX, pY) => {
